...
def process_txs(txs: list[dict[str, Any]], address: str, page: str) -> None:
    for tx in txs:
        tx['page'] = page
...
def get_transactions_missed():
    ...
    # The last tweet was at the round below
    # Post Link: https://x.com/AFWalletWatcher/status/2000949989959168113?s=20 
    # Trigger Tx: https://explorer.perawallet.app/tx/7MLA6AHUCFYTW3OJKX7SNFVS2NRUDP3RGIMZ6SKTATOXWUJSBFJA/
    last_round_tweeted = 56538330 

    start_round = last_round_tweeted + 1
    current_round = cast(dict[str, Any], algorand.client.algod.status())['last-round']

    for wallet in foundation_market_wallets:
        next_page = None
        while True:
            address = wallet
            label = foundation_market_wallets[address]
            if next_page == None:
                response = indexer.search_transactions(min_round=last_round_tweeted, address=wallet, limit=1000)
                next_page = response.get('next-token', None)
                txs = response.get('transactions', [])
            else:
                response = indexer.search_transactions(min_round=last_round_tweeted, next_page=next_page, address=wallet, limit=1000)
                next_page = response.get('next-token', None)
                txs = response.get('transactions', [])

            if next_page == None:
                break
            process_txs(txs, wallet, next_page)
            time.sleep(1)
